...
// Test with just 3 assets first
const smallMultiAssetInput = "equity:AAPL,fx:EUR,cfd:XAU:USD";

async function testSmallMultiAssets() {
    console.log("🧪 Testing Oracle Program with Small Multi-Asset Data Request\n");
    console.log(`Oracle Program ID: ${ORACLE_PROGRAM_ID}\n`);
    console.log(`📊 Testing 3 assets: ${smallMultiAssetInput}\n`);
    ...
    try {
        console.log(`🔄 Submitting small multi-asset data request...`);
        ...
        const dataRequestInput: PostDataRequestInput = {
            ...
            execInputs: Buffer.from(smallMultiAssetInput),
            tallyInputs: Buffer.from([]),
            memo: Buffer.from(`Test DR for 3 assets - ${new Date().toISOString()}`),
            gasPrice: 20000n,
            gasLimit: 500000n
        };
...
// Run the test
testSmallMultiAssets().catch(console.error); 